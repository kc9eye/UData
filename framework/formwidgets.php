...
Class FormWidgets {
    ...
    /**
     * @var String $third_party The URL to the third-party apps storage
     */
    public $third_party;
    ...
    /**
     * @var Boolean $uploadhack Whether to add the file upload hack script
     */
    protected $uploadhack;

    /**
     * @var Boolean $wysiwyg Whether to include the textArea 'tinymce' editor
     */
    protected $wysiwyg;
    ...
    public function __construct ($scripts) {
        ...
        $this->third_party = dirname(dirname($scripts)).'/third-party';
    }
...
<div style='margin:10px'>
    ...
    public function newForm ($header = null, $uri = null, $method = 'post', $id = null) {
        ...
        if (!is_null($header)) {
            echo "<div class='center-text center-block'><h2>{$header}</h2></div>";
        }
        echo "<form method='{$method}' class='form-horizontal' id='{$this->formid}'";
        if (!is_null($uri)) {
            echo " action='{$uri}'";
        }
        echo ">";
    ...
    public function endForm () {
        ...
        if ($this->validate) {
            ...
            echo "    $('#{$this->formid}').validate(".str_replace('"','',json_encode(['rules'=>$this->rules],JSON_FORCE_OBJECT)).");";
            echo "  });";
            echo "</script>";            
        }
        if ($this->wysiwyg) {
...
<?php
     ...
     * @param Mixed $cancel Optional hyperlink to use for a cancel button, or optional array of 
     * `['label'=>'btn_label','url'=>'onlcick_url',type'=>'default|primary|info|warning|danger|success']`
     * @return Void
     */
...
<?php
     ...
     * @param String $value This parameter is not used. If you require this parameter consider using `standardFileUpload`
     * @param Boolean $multiple Whether or not to accept multiple files for upload
     * @param Boolean $required Whether to validate this field for emptiness
     * @param String $helpBlock A string of optional help text for the field
     * @param Mixed $required Optional `true` or array of jquery.validate() properties
     * @todo rewrite for PHP instead of HTML
     */
    public function fileUpload ($name, $label = 'File', $value = null, $multiple = false, $required = false, $helpBlock = null) {
        $this->uploadhack = true;
        if ($required) $this->addValidation($name,$required);
        $name = $multiple ? "{$name}[]" : $name;
        echo "<div class='form-group'>";
        echo "<div class='row'>";
        echo "<div class='col-md-3'></div>";
        echo "<div class='col-xs-12 col-md-1 right-text'><label>{$label}:</label></div>";
        echo "<div class='col-xs-12 col-md-5'>";
     ...
     * @param String $default An optional default value for the unchecked checkbox.
     * @return Void 
     */
    public function checkBox ($name, Array $label, $value, $required = false, $helpBlock = null, $default = null) {
        if ($required) {
            $this->addValidation($name, $required);
        }
        echo "<div class='form-group'>";
...
<?php
     ...
     * @param Array Optional array in the same form as `$value` array. The given option will be the selected default value 
     * of the select box.
     */
    public function selectBox ($name, $label, Array $value, $required = false, $helpBlock = null, $default = null) {
        if ($required) {
            $this->addValidation($name, $required);
        }
        echo "<div class='form-group'>";
        ...
        if (!is_null($default)) echo "<option value='{$default[0]}' selected='selected'>{$default[1]}</option>";
    ...
    /**
     * Adds a search box input capture to the form
     * 
     * Outputs a search box complete with search button to the form,
     * styled in the centered form style. This search box is it's own form.
     * This method outputs a completely new form and is intended to be used 
     * as a stand alone search box, separate from any other forms on the page.
     ...
     */
    public function searchBox ($name, $label, $value = null, $required = false, $helpBlock = null) {
        if ($required) {
            $this->addValidation($name, $required);
        }
        $this->newForm();
    ...
    /**
     * Outputs a formated textarea to the stream
     * 
     * @param String $name The `$_REQUEST` name given to the handler.
     * @param String $id The UI display name for the widget.
     * @param String $value Optional pre-inserted value.
     * @param Mixed $required Optionally either true or an indexed array of jQuery.validate() options
     * @param String $helpBlock Optional string of help text for the widget
     * @return Void Output is directed to the stream, and this widget returns no value
    ...
    /**
     * Outputs a non bootstrap standard HTML 5 file upload to the stream
     * @param String $name The name given to the $_REQUEST variable
     * @param String $label The label for the form group
     * @param String $value Any pre loaded value for the upload
     * @param Boolean $multiple Default false, whether to accept multiple file or not
     * @param Mixed $required Either true if validatation required, or array of jquery validate rules
     * @param String $helpBlock Help block information default is null
     */
    public function standardFileUpload ($name, $label = 'File', $value = null, $multiple = false, $required = false, $helpBlock = null) {
        if ($required) $this->addValidation($name,$required);
        $name = $multiple ? "{$name}[]" : $name;
        echo "<div class='form-group'>";
        echo "<div class='row'>";
        echo "<div class='col-md-3'></div>";
        echo "<div class='col-xs-12 col-md-1'><label class='control-label' for='{$name}'>{$label}:</label></div>";
        echo "<div class='col-xs-12 col-md-5'><input class='form-control' type='file' name='{$name}' ";
