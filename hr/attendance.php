...
function editAttendanceDisplay() {
    ...
    <form id="editForm">
        ...
        <select class="form-control" name="description">
            <option value="Absence"',($row['description'] == "Absence")?' selected = "selected"':"",'>Absence</option>
...
function amendAttendanceRecord() {
    global $server;
    $sql = 'update missed_time set occ_date = :occ_date, arrive_time = :arrive_time, leave_time = :leave_time, points = :points, comments = :comments, description = :description
    where id = :id';
    $pntr = $server->pdo->prepare($sql);
    try {
        $insert = [
...
function printDisplay () {
    ...
    echo "<table border='1'>\n";
    echo "<tr><th>Date</th><th>Arrived Late</th><th>Left Early</th><th>Absent</th><th>Type</th><th>Comments</th></tr>\n";
    foreach($emp->Attendance as $row) {
        $absent = ($row['absent'] == 'true') ? 'Yes' : 'No';
        //$excused = ($row['excused'] == 'true') ? 'Yes' : 'No';
        echo "<tr><td>{$row['occ_date']}</td><td>{$row['arrive_time']}</td><td>{$row['leave_time']}</td>";
        echo "<td>{$absent}</td><td>{$row['description']}</td><td>{$row['comments']}</td>";
        echo "</tr>\n";
    }
    echo "</table>\n";
